...
Then run the project's own test tiers against the implemented code — tests are run, never faked; never record a result you did not observe. Infer how the project runs tests (task-runner scripts, `Makefile`, test dirs/config) and classify tiers: **unit / smoke** (no external dependencies) versus **integration / e2e / api** (real dependencies: DB, HTTP API, message broker, browser, external service). Run the unit tier, then every real-test tier that is runnable locally without credentials, remote side effects, or paid calls — do not stop at unit tests when real ones are runnable. Record any tier that cannot run locally as **Blocked** with the reason in `{spec_file}`'s `## Verification` section — a Blocked tier is reported, never faked, and never a reason to HALT. A failure in any tier that did run follows the same rule as above: fix it if possible, otherwise HALT with status `blocked` and blocking condition `implementation verification failed`. NEVER weaken, skip, or delete a test to make the suite pass.
