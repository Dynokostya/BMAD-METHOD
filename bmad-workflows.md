...
- **Platform Overrides on every skill.** Injected through the customization system: each skill's `customize.toml` ships `activation_steps_prepend` entries (rendered into the workflow on activation), and the few skills without that hook (`bmad` hub, customize, advanced-elicitation, agent personas) carry a `### Platform Overrides` block in SKILL.md. The override texts live in `skills/_shared/` (`claude-code-overrides.md` / `codex-overrides.md`) and are referenced as `_bmad/_shared/...` — the Claude Code / Codex behavioral tuning that defines this fork (AskUserQuestion for all questions, TaskCreate/update_plan tracking, typed subagent delegation). The load gate is still the classic installer's `_bmad/_config/ides/claude-code.yaml` / `codex.yaml` marker; upstream v6.12+ retired that installer (skills now arrive via `npx skills add` or the plugin marketplaces, and `bmad setup` writes `_bmad/`), so the gate and the delivery of the `_shared` files need re-homing on the new layout.
